...
- API路径：
  `/api/sceneCollecting/sceneInfo/saveToCache`
...
- API路径：
  `/api/sceneCollecting/sceneInfo/getFromCache`
...
- API路径：
  `/api/sceneCollecting/sceneInfo/save`
...
- API路径：
  `/api/sceneCollecting/sceneInfo/summit`
...
- 请求体：
  ...
  ```json
  {
    "investigationId": "勘验ID，为空时表示所有数据未进行入库操作",
