...
top.restPath=localData.get('restPath');
...
top.sysParams=localData.get('sysParams')||{};


importing('utility.js','main.js','fullscreen','socket', 'dict',typeof require=='function'?'client.js':'',function(){
    ...
    function molReorganize(){
        ...
        console.info(LIMITS.pages);

        views.each(function(item){
            $state.on(item.pageNo,{
                view:item.direct
            })
        });
    ...
    function menuInit(){
        ...
        $filter('iconClass',function(){
            var dic={
                ...
                '系统管理':'icon-cogs fs15'
                //'我的应用':'glyphicon glyphicon-show_big_thumbnails',
                //'案件勘验':'glyphicon glyphicon-tags',
                //'物证保管':'glyphicon glyphicon-sampler',
                //'检验鉴定':'icon-beaker ',
                //'质量检查':'glyphicon glyphicon-check',
                //'统计考核':'icon-bar-chart',
            };
        ...
        rootMenu.find('a[direct*=".htm"]').on('click',function(){
            var $this=$(this);
        ...
        //添加三级菜单icon
        var thridLinks=rootMenu.find('.grade3 a');
        thridLinks.each(function(i,item){
            $(item).addClass('accordion-toggle').html('<span class="fa fa-dot-circle-o"></span><span class="sidebar-sub-title">'+$(this).html()+'</span>');
        })
        thridLinks.click(function(){
            thridLinks.removeClass('current');
            $(this).addClass('menu-open2 current');
        })

        rootMenu.on('mouseenter','body.sb-l-m .sidebar-menu > li',function(){
