...
var editInputTemplateAct = makeAct('sceneCollecting/inputTemplate/upd');
var inputTemplateAddListAct = makeAct('sceneCollecting/inputTemplate/getBase');
var addInputTemplateAct = makeAct('sceneCollecting/inputTemplate/add');
var inputTemplateDelAct = makeAct('sceneCollecting/inputTemplate/delLogic');
var wbxxmbglListAct = makeAct('sceneCollecting/textTemplate/query');
var textTemplateDelAct = makeAct('sceneCollecting/textTemplate/delLogic');
var textTemplateAddAct = makeAct('sceneCollecting/textTemplate/add');
var textTemplateEditAct = makeAct('sceneCollecting/textTemplate/upd');
...
*/
        ...
        $('#user-query-condition').on('ex-reset',resetForQuery).on('ex-query', function(e){ queryForUser() });
...
function sysXtdhFn() {
    importing('dict', function () {
        ...
        //回调函数
        function pageBackXtdh(res) {
            // pageData = data;
            $template('#xtdh-info-table tbody', res.data, function (item, i) {
...
function sysInputTempLateFn() {
    importing('datepicker','dict',function(){
        ...
        function queryForInputTemplate(){
            $('#inputTemplate-query-result').pagingList({
        ...
        $('#add-input-template').on('click',function () {
            inputTemplateAdd();
        });
...
//文本模板管理
function sysTextTempLateFn() {
    importing('dict',function(){

    	$(".dict").dict();

        $('[dict-id="edit-case-type"]').on('ex-change',function(){
            getTextTemplateLabel('edit',$('#edit-div').data('paramObj'));
        });
        $('[dict-id="add-case-type"]').on('ex-change',function(){
        ...
        function queryForTextTemplate(){
            ...
            $('#textTemplate-query-result').pagingList({
        ...
        //新增
        $('#wbxxmbgl-add-btn').on('click',function () {
            textTemplateAdd();
        });
        //修改
        ...
        $('.wrap-win').on('click','[dict-name="commonFlag"] input:radio',function () {
            ...
            $ajlbWrap.children('dict').trigger('ex-change');
        });
...
(function(){
    ...
    var initFn=window.dash2camel(pageNo)+'Fn';
    logEx([initFn,typeof window[initFn]]);
    //pageNo && typeof window[initFn]=='function' && initFn(pageNo);
})();

window.MODULE={
    sysUserMngFn:sysUserMngFn,
    sysRoleMngFn:sysRoleMngFn
}