    ...
    $.each(_target, function (i, o) {
        ...
        if (type === _dictConfig.select.name) {
            ...
            if (multiple) {
                ...
                initValueArr = initValue instanceof Array ? initValue : initValue.split(',');
                ...
                initValue = (initValue instanceof Array) ? initValue[0] : initValue;
...
function setHidVal(target,multiple) {//设置隐藏域的值
    ...
    var radioCheckVal = '';
    target.children('input:checkbox').each(function (i,ele) {
        var $ele = $(ele);
        var $eleVal = $ele.val();
...
function initTreeDict(target, data, cmUseData,cb) {
    ...
    target.on('change','input:radio',function () {
        ...
        $body.trigger('ex-change',['[dict-id="{0}"]'.format(dictId),hidVal,hidChVal]);
    });
    target.on('change','input:checkbox',function () {
        var valArr = setHidVal(target,multiple);
        $body.trigger('ex-change',['[dict-id="{0}"]'.format(dictId),valArr[0],valArr[1]]);
    });
    if(target.children('label').first().text()=='不限'){
        target.children('input:checkbox').first().on('click',unlimitedClickFn).siblings('input:checkbox').on('click',notUnlimitedClickFn);
    }
...
function treeDictOk() {
    ...
    $('#' + currentDictConfig.dictId).trigger('ex-change',[returnValue]);
...
function selectQueryDict(key, value) {
    if (currentDictConfig.multiple) {
        ...
        $('#' + currentDictConfig.dictId + '_displayValue').removeClass('validatebox-invalid');
        $('#' + currentDictConfig.dictId).trigger('ex-change',[value]);
        window.currentDictNode = null;
        window.currentDictConfig = null;
        window.dictWin.$close();
        //window.dictWin.find('.panel-tool-close').click();
