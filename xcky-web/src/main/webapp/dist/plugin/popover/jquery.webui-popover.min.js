 ...
 *  Under MIT License
 */
typeof $style==='function'&& $style(getDistPath()+'plugin/popover/jquery.webui-popover.min.css');
    ...
    WebuiPopover.prototype = {
        ...
        init: function () {
            ...
            if (this.options.trigger==='click'){
                this.$element.off('click').on('click',$.proxy(this.options.toggle?this.toggle:this.notoggle,this));
            }else if (this.options.trigger==='mouseenter'){
                this.$element.off('mouseenter mouseleave')
                    .on('mouseenter',$.proxy(this.mouseenterHandler,this))
                    .on('mouseleave',$.proxy(this.mouseleaveHandler,this));
            }else{
                $.proxy(this.options.toggle?this.toggle():this.notoggle(),this);
            }
            this._poped = false;
            this._inited = true;
        },
        ...
        show:function(){
            ...
            if (this.options.type==='iframe'){
                var $iframe = $target.find('iframe').attr('scrolling','no')//.parent().css('overflow-x','hidden');//mark yao
        ...
        getPlacement:function(pos,targetHeight){
            var
                ...
                pageX2 = window.innerWidth-pageX;
                pageY2= window.innerHeight-pageY;
                arrowSize = 20;
            ...
            //targetHeight=targetHeight-180;
            info([pageY,targetHeight+arrowSize])
            if (placement==='auto'){
            ...
            //mark Yao
            var needSize=targetHeight+arrowSize+10;

            if(pageY<needSize && pageY2<needSize){
                placement = pageX2>pageX?'right':'left';
                pageY2>pageY? (placement+='FixDown'):(placement+='FixUp')
            }
            return placement;
        },
        ...
        getTargetPositin:function(elementPos,placement,targetWidth,targetHeight){
            var pos = elementPos,
                ...
                position={},
                arrowOffset={},
                arrowSize = this.options.arrow?11:0;
            switch (placement) {
                case 'bottom':
                ...
                //mark yao
                case 'leftFixUp':
                    position = {top: pos.top-25 + pos.height / 2 - targetHeight / 2, left: pos.left - targetWidth -arrowSize};
                    arrowOffset = {top: targetHeight / 2 + 25};
                    break;
                case 'rightFixUp':
                    position = {top: pos.top-25 + pos.height / 2 - targetHeight / 2, left: pos.left + pos.width};
                    arrowOffset = {top: targetHeight / 2 + 25};
                    break;
                case 'leftFixDown':
                    position = {top: pos.top +50 + pos.height / 2 - targetHeight / 2, left: pos.left - targetWidth -arrowSize};
                    arrowOffset = {top: targetHeight / 2 - 50};
                    break;
                case 'rightFixDown':
                    position = {top: pos.top +50 + pos.height / 2 - targetHeight / 2, left: pos.left + pos.width};
                    arrowOffset = {top: targetHeight / 2 - 50};
                    break;
                case 'left':
